"""
RSIScalpingTSLA — bar backtest mirroring main.mq5 inputs.
...
class StrategyParams:
  rsi_period: int = 14
  rsi_overbought: float = 32
  rsi_oversold: float = 86
  rsi_target_buy: float = 100
  rsi_target_sell: float = 24
  bars_to_wait: int = 34
  lot_size: float = 5
...
def run_backtest(df, symbol, params: StrategyParams, costs, period_label):
  ...
  return run_single_position(
    df, symbol, point, costs, params.lot_size, STRATEGY_ID, "M20", period_label, p, params.initial_balance, on_bar
  )
...
def main() -> None:
  ...
  try:
    ...
    df = load_bars(symbol, mt5.TIMEFRAME_M20, start, end)
