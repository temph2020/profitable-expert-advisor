"""
SimpleTrendlineBTCUSD — bar backtest mirroring main.mq5 inputs.
...
STRATEGY_ID = "SimpleTrendlineBTCUSD"
...
class StrategyParams:
  ma_period: int = 150
  touch_tolerance_pts: float = 170
  break_buffer_pts: float = 90
  lot_size: float = 0.10
...
def run_backtest(df, symbol, params: StrategyParams, costs, period_label):
  ...
  hdf = df.resample("4h").agg({"open": "first", "high": "max", "low": "min", "close": "last"}).dropna()
  ma = calculate_ema(hdf["close"], params.ma_period).to_numpy()
  htimes = hdf.index.to_numpy()
  hcloses = hdf["close"].to_numpy()
  touch_tol = params.touch_tolerance_pts * point
  break_buf = params.break_buffer_pts * point
  p = params.to_dict()
  ...
  def on_bar(i, st, open_pos, close):
    ...
    if hidx < params.ma_period + 5:
      return
    crosses_t, crosses_p = [], []
    for j in range(hidx, params.ma_period + 2, -1):
      if j >= len(ma) - 1:
        continue
      d0, d1 = hcloses[j] - ma[j], hcloses[j + 1] - ma[j + 1]
    ...
    def _secs(delta) -> float:
      ...
      return float(delta.astype("timedelta64[s]").astype(float))

    xs = np.array([_secs(t - t0) for t in crosses_t[::-1]])
    ...
    t1, t2 = df.index[i - 1], df.index[i - 2]
    line1 = a * _secs(t1 - t0) + b
    line2 = a * _secs(t2 - t0) + b
    ...
    if st.side:
      return
    if a > 0 and lo <= line1 + touch_tol and cl1 > line1 and cl1 > op1 and cl2 >= line2 - touch_tol:
      open_pos(i, "BUY", mid)
    elif a < 0 and hi >= line1 - touch_tol and cl1 < line1 and cl1 < op1 and cl2 <= line2 + touch_tol:
      open_pos(i, "SELL", mid)
