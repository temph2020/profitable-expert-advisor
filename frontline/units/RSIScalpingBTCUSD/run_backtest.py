"""
RSIScalpingBTCUSD — bar backtest mirroring main.mq5 inputs.
...
class StrategyParams:
  ...
  rsi_oversold: float = 73
  rsi_target_buy: float = 88
  rsi_target_sell: float = 48
  bars_to_wait: int = 6
  lot_size: float = 0.1
  use_reversal_escape: bool = True
  reversal_atr_period: int = 14
  reversal_adverse_atr_mult: float = 5.25
  reversal_signs_required: int = 2
  reversal_rsi_velocity: float = 16.0
  reversal_body_atr_mult: float = 5.1
...
def run_backtest(df, symbol, params: StrategyParams, costs, period_label):
  ...
  return run_single_position(
    df, symbol, point, costs, params.lot_size, STRATEGY_ID, "H1", period_label, p, params.initial_balance, on_bar
  )
